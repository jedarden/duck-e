...
var ducke = (() => {
  ...
  var WebRTC = class {
    ...
    onDisconnect;
    onMessage;
    constructor(duckeSocketUrl, microphone) {
      this.duckeSocketUrl = duckeSocketUrl;
    ...
    async connect() {
        ...
        _dc.addEventListener("message", (e) => {
          ...
          if (message.type && message.type.includes("function")) {
            ...
            try {
              ...
              console.error(
                "Error sending function message to DUCK-E backend",
                error
              );
              webRTC.close();
            }
      ...
      this.ws = new WebSocket(this.duckeSocketUrl);
      ...
      this.ws.onmessage = async (event) => {
        try {
          const message = JSON.parse(event.data);
          console.info("Received Message from DUCK-E backend", message);
